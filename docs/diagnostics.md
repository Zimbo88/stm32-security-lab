...
addresses, retained fault PC/LR details, raw register snapshots, and detailed
runtime telemetry. It is disabled by default and is only emitted when EXP066 is
built with `RSM_RESTRICTED_DIAGNOSTICS=1`. Secret output is never available.
...
`rsm status` emits the phase-1 Runtime Security Monitor key-value schema. It
...
basic runtime checks into a public evidence summary.
