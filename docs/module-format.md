...
The capability table must begin immediately after the fixed header. The payload
must begin immediately after the capability table. There is no accepted unsigned
or ignored gap between the fixed header, capability table, and payload.
...
Parsers reject malformed ranges, overlap, gaps between the header, capability
table, and payload, trailing bytes after the payload, duplicate capabilities,
unsupported types, nonzero flags, bad versions, rollback versions, CRCs,
hashes, signatures, and reserved bytes.
