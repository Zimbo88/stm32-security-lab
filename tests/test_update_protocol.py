...
UPDATE_PROTOCOL = ROOT / "tests" / "update_protocol"


def run_make(*args: str) -> subprocess.CompletedProcess[str]:
    return subprocess.run(
        ["make", "-C", str(UPDATE_PROTOCOL), *args],
        text=True,
        capture_output=True,
        check=False,
    )
...
def test_update_protocol_host_tests() -> None:
    assert_success(run_make("clean", "test"))


def test_update_protocol_host_tests_with_sanitizers_when_supported() -> None:
