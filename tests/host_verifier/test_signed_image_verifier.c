...
static void test_rfc8032_empty_message_vector(void)
{
    static const uint8_t public_key_vector[32] = {
    ...
    expect_int(
        "RFC8032 empty Ed25519 vector",
        0,
        crypto_ed25519_check(signature_vector, public_key_vector, NULL, 0U)
    );
    crypto_sha512(digest, NULL, 0U);
    expect_int(
        "SHA-512 empty message vector",
        0,
        memcmp(digest, sha512_empty, sizeof(digest))
    );
}
...
int main(void)
{
    ...
    test_rfc8032_empty_message_vector();
